...
struct periodic{
  ...
  dynpart* get_dynpart(int i){return dynparts[i];}
  void switch_torso_penalty(bool force, bool torque);
void check_solve_ft(); // temporary
  double* get_masses(){return masses;}
  int get_number_of_dynparts(){return dynparts.size();}
  int* get_parentis(){return parentis;}
  int* get_footis(){return footis;}
  int get_nfeet(){return nfeet;}
  int get_nt(){return n_t;}
  ...
  void get_contforce_stat(double* stat){stat[0] = min_cfz; stat[1] = max_mu;}
  void compute_torques_over_period();
  double* get_computed_torques(int i){return computed_torques[i % n_t];}
