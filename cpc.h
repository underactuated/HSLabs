...
public:
  ...
  void set_current_state(double* config, double* dconfig);
...
void get_torques0(double* torques); // temporaty/experimental
  //void set_poscontrol_flag(bool value); // experimental
...
private:
  ...
  void compute_prox_loss_over_tps(list<int>& tpis, map<double,cand>& loss_cands);
  double prox_loss(double t0, double s, double cc0, double cc1);
  void candidates(list<cand>& cands);
  void controls(double* torques, double k, cand& can);
  void controls(VectorXd& tau, double k, cand& can);
  void controls1(VectorXd& tau, VectorXd& del_tau, double k, cand& can);
  void get_tau(int tpi, VectorXd& tau);
  int cost(double k, list<cand>& cands, MatrixXd& cand_taus);
  int cost2(double k, list<cand>& cands, MatrixXd& cand_taus);
  int cost1(double k, list<cand>& cands, MatrixXd& cand_taus);
